...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('form_registro_auto').addEventListener('submit', function(event) {
        ...
        var fileInput = document.getElementById("img_vehiculo");
        var file = fileInput.files[0];


        formData.append("img_vehiculo",file);


        fetch('http://localhost/App_transports/public/crear_vehiculo', {
            method: 'POST',
            body: formData
        })
        .then(response => response.json())
        .then(data => {
            ...
            if (data.status == 0) {
                ...
                $('#modal_info_conductor').modal('hide');
